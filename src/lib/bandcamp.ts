...
// --- discography (label / artist catalogue) ---------------------------------
// A band's /music page carries the whole catalogue in one `data-client-items`
// JSON blob, so a single request gets every release URL — no pagination.
// Same stripped-page trap as above: without browser headers the attribute
// isn't there, so this reuses BROWSER_HEADERS.
...
/**
 * Parses a /music page. `origin` resolves the relative `page_url`s an artist
 * page uses (a label page uses absolute cross-subdomain ones instead).
 */
export function parseDiscography(html: string, origin: string): Discography | null {
  const raw = attr(html, "data-client-items");
  if (!raw) return null;

  let items: RawClientItem[];
  try {
  ...
  if (band) {
    try {
      bandName = (JSON.parse(band) as { name?: string }).name ?? "";
    } catch {
      // leave blank — the caller falls back to the hostname
    }
  }

  const releases = items
    .filter((i) => i.page_url && (i.type === "album" || i.type === "track"))
    .map((i) => {
      const u = new URL(i.page_url as string, origin);
      return {
        url: `${u.origin}${u.pathname}`, // drop ?label=…&tab=music — same page, stable key
        title: i.title ?? "",
        artist: i.artist ?? bandName ?? null,
        artUrl: artUrlFromId(i.art_id),
      };
    });

  return { bandName, items: releases };
}

export async function fetchDiscography(origin: string): Promise<Discography | null> {
  let res: Response;
  try {
    res = await fetch(`${origin}/music`, { headers: BROWSER_HEADERS, redirect: "follow" });
  } catch {
    return null;
  }
  if (!res.ok) return null;
  return parseDiscography(await res.text(), origin);
}
